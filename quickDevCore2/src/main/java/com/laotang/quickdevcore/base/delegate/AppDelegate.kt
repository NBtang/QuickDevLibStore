...
class AppDelegate : App,AppLifecycle, KodeinAware {
    ...
    internal fun init(context: Context):AppDelegate{
        ...
        this.mAppLifecycleList.add(AppCoreLifecycle())
        this.mModules?.run {
            //根据优先级排序
            sortBy {
                it.priority
    ...
    override fun onCreate(application: Application) {
        ...
        if(AppLifecycleHelper.treeCount()>0){
            AppLifecycleHelper.onCreate(application)
            AppLifecycleHelper.uprootAll()
        }
    }
